...
export function DashboardSidebar({ isCollapsed, onToggleCollapse, navItems, user }: SidebarProps) {
    return (
        ...
        >
            ...
            <Link to="/" className="h-16 flex items-center px-5 gap-3 border-b border-border/30">
            ...
            <nav className="flex-1 px-3 py-4 space-y-1">
                {navItems.map((item) => (
                    <NavLink
                        ...
                        className={({ isActive }) =>
                            `flex justify-center items-center gap-3 rounded-lg px-3 py-2 transition-all relative group/nav ${isActive
